...
from experiments.nvae.hyper_model import HyperAutoEncoder
...
def main(args):
  ...
  model = HyperAutoEncoder(args, None, arch_instance, hyper_cfg=cfg)
  ...
  #     eps=1e-3)
  cnn_optimizer = torch.optim.Adamax(
      model.parameters(),
      args.learning_rate,
      weight_decay=args.weight_decay,
      eps=1e-3)
  ...
  for epoch in range(init_epoch, args.epochs):
    ...
    train_nelbo, global_step = train(train_queue, model, cnn_optimizer, grad_scalar,
                                     global_step, warmup_iters, None, logging)
    ...
    if epoch % eval_freq == 0 or epoch == (args.epochs - 1):
      ...
      for beta in betas:
        with torch.no_grad():
          ...
          for t in [1.0]:
            logits = model.sample(num_samples, t, beta)
            ...
            wandb.log({'generated_%0.1f_{}'.format(beta) % t: wandb.Image(output_tiled)})

        valid_neg_log_p, valid_nelbo, valid_recon, valid_kl = \
          test(valid_queue, model, num_samples=10, args=args, logging=logging, value=beta)
        ...
        log_dict["val/neg_log_p_{}".format(beta)] = valid_neg_log_p
  ...
  # Final validation
  # valid_neg_log_p, valid_nelbo, _, _ = \
  #   test(valid_queue, model, num_samples=1000, args=args, logging=logging)
...
def train(train_queue,
  ...
  for step, x in enumerate(train_queue):
    ...
    with autocast():
      logits, log_q, log_p, kl_all, kl_diag, sample_dict = model.sample_forward(x)
...
def test(valid_queue, model, num_samples, args, logging, value):
  ...
  for step, x in enumerate(valid_queue):
    ...
    with torch.no_grad():
      ...
      for k in range(num_samples):
        logits, log_q, log_p, kl_all, _, sample_dict = model.fixed_forward(x, value)
        output = model.decoder_output(logits)
