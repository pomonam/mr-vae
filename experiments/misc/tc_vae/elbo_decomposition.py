...
def estimate_entropies(qz_samples, qz_params, q_dist):
  ...
  qz_samples = qz_samples.index_select(
      1, Variable(torch.randperm(qz_samples.size(1))[:10000].to(DEVICE)))
  ...
  while k < S:
    ...
    marginal_entropies += (math.log(N) -
                           logsumexp(logqz_i, dim=0, keepdim=False).data).sum(1)
    # computes - log q(z) summed over minibatch
    logqz = logqz_i.sum(1)  # (N, S)
    joint_entropy += (math.log(N) -
                      logsumexp(logqz, dim=0, keepdim=False).data).sum(0)
...
def analytical_NLL(qz_params, q_dist, prior_dist, qz_samples=None):
  pz_params = Variable(
      torch.zeros(1).type_as(qz_params.data).expand(qz_params.size()),
      volatile=True)
...
def elbo_decomposition(vae, dataset_loader):
  ...
  for xs in dataset_loader:
    ...
    for _ in range(S):
      ...
      logpx += vae.x_dist.log_density(
          xs, params=x_params).view(batch_size, -1).data.sum()
  # Reconstruction term
  logpx = logpx / (N * S)
