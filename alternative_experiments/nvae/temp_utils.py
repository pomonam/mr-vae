...
def kl_balancer_coeff(num_scales, groups_per_scale, fun):
  if fun == 'equal':
    ...
    coeff = torch.cat(
      [np.square(2 ** i) / groups_per_scale[num_scales - i - 1] * torch.ones(groups_per_scale[num_scales - i - 1]) for i
       in range(num_scales)], dim=0).cuda()
  else:
...
def kl_coeff(step, total_step, constant_step, min_kl_coeff, beta):
  return max(min((step - constant_step) / total_step, beta), min_kl_coeff)
