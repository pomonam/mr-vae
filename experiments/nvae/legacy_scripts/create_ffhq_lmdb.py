...
def main(split, ffhq_img_path, ffhq_lmdb_path):
  ...
  with env.begin(write=True) as txn:
    for i in file_ind:
      ...
      im = np.array(
          im.getdata(), dtype=np.uint8).reshape(im.size[1], im.size[0], 3)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(
      'FFHQ LMDB creator. Download images1024x1024.zip from here and unzip it \n'
  ...
  parser.add_argument(
      '--ffhq_img_path',
      type=str,
      default='/data1/datasets/ffhq/images1024x1024',
      help='location of images from FFHQ')
  parser.add_argument(
      '--ffhq_lmdb_path',
      type=str,
      default='/data1/datasets/ffhq/ffhq-lmdb',
