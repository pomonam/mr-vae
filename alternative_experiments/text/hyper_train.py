...
from alternative_experiments.text.model_pipeline import build_hyper_criterion
from alternative_experiments.text.model_pipeline import build_hyper_model
...
def hyper_evaluate(data_name, model, criterion, epoch, name, delta=0.01):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for sample in sample_lst:
            loader = build_input_queue(data_name, name, args.batch_size, DEVICE)
...
def hyper_train(data_name, model, biq, criterion, optimizer, cfg, hyper_cfg):
    ...
    while epoch < cfg.total_epochs:
        ...
        if do_evaluate:
            hyper_evaluate(data_name, model, criterion, epoch, "train_eval")
            hyper_evaluate(data_name, model, criterion, epoch, "test")
        ...
        model.train()
        loader = biq(data_name, "train", cfg.batch_size, DEVICE)
        p_bar = tqdm.tqdm(loader)
        metric_dict = initialize_metric(criterion.get_metric_lst())
...
def main():
    ...
    model = build_hyper_model(args.data_name,
                              hyper_cfg,
                              DEVICE)
    ...
    hyper_evaluate(args.data_name, model, criterion, cfg.total_epochs, "test")
