...
def main(eval_args):
  ...
  if eval_args.eval_mode == 'evaluate':
    ...
    num_iter = int(np.ceil(total_samples /
                           num_samples))  # num iterations per gpu
      ...
      for ind in range(num_iter):  # sampling is repeated.
        ...
        if visualize:
          ...
          file_path = os.path.join(
              eval_args.save,
              'gpu_%d_samples_%d.npz' % (eval_args.local_rank, ind))
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--checkpoint',
      type=str,
      default='/tmp/expr/checkpoint.pt',
  ...
  parser.add_argument(
      ...
      type=str,
      default='sample',
      choices=['sample', 'evaluate', 'evaluate_fid'],
  ...
  # DDP.
  parser.add_argument(
      '--local_rank', type=int, default=0, help='rank of process')
  parser.add_argument(
