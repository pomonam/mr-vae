...
def evaluate(model, loader, criterion, epoch, name, device, delta=0.01):
  ...
  with torch.no_grad():
    ...
    for batch in p_bar:
      ...
      # mutual_info = model.calc_mi(batch["data"])
      # mi_meter.update(mutual_info, batch["data"].size(0))

      loss_dict = criterion(recon_x=output_dict["reconstruction"],
                            x=output_dict["data"],
                            mu=output_dict["mu"],
...
def train(model, train_loader, test_loader, criterion, optimizer, scheduler, device, cfg):
