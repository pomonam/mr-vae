...
class LinearVae(nn.Module):
    ...
    def forward(self, x, beta=-1, num_iwae=1):
        ...
        loss_dict = {
            "loss": elbo,
            "distortion": -torch.mean(likelihood),
            "rate": torch.mean(kl)
        }
        return elbo, output_dict, loss_dict
...
from src.hyper.layers import HyperLinear
from src.hyper.layers import HyperModule
...
class LinearHyperVae(LinearVae):
    ...
    def sample_beta(self, x: torch.Tensor, warmup=False):
        ...
        if self.hyper_config.sample_type == "log_uniform":
            a = math.log(a)
            b = math.log(b)
            sample_dict["net_beta"] = torch.FloatTensor(batch_size, 1).uniform_(
                a, b).to(device)
