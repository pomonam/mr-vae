...
class PixelCNNBlock(nn.Module):
    def __init__(self, in_channels, kernel_size):
        ...
        self.main = nn.Sequential(
            ...
            nn.ELU(),
            MaskedConv2d(self.mask_type, out_channels, out_channels, out_channels, kernel_size,
...
class MaskABlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, masked_channels):
        ...
        self.layers = nn.Sequential(
            MaskedConv2d(self.mask_type, masked_channels, in_channels, out_channels, kernel_size,
                         padding=padding, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ELU(),
        )
