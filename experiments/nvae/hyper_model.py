...
from experiments.nvae.hyper_ar_operations import ARConv2d
from experiments.nvae.hyper_ar_operations import ARInvertedResidual
from experiments.nvae.hyper_ar_operations import ELUConv as ARELUConv
from experiments.nvae.hyper_ar_operations import mix_log_cdf_flow
from experiments.nvae.hyper_ar_operations import MixLogCDFParam
from experiments.nvae.hyper_neural_operations import Conv2D
from experiments.nvae.hyper_neural_operations import DecCombinerCell
from experiments.nvae.hyper_neural_operations import EncCombinerCell
from experiments.nvae.hyper_neural_operations import get_skip_connection
from experiments.nvae.hyper_neural_operations import OPS
from experiments.nvae.hyper_neural_operations import SE
...
class HyperCell(nn.Module):
  ...
  def __init__(self, Cin, Cout, cell_type, arch, use_se, hyper_cfg):
    ...
    self.skip = get_skip_connection(
        Cin, stride, affine=False, channel_mult=CHANNEL_MULT)
    self.use_se = use_se
    self._num_nodes = len(arch)
    self._ops = nn.ModuleList()
    self._hyper_ops = nn.ModuleList()
    for i in range(self._num_nodes):
      ...
      self._ops.append(op)
      if "dec" in cell_type:
        hyper_op = get_hyper_layer(Cout, hyper_cfg, decoder=True)
      else:
        hyper_op = get_hyper_layer(Cout, hyper_cfg)
      self._hyper_ops.append(hyper_op)
...
class CellAR(nn.Module):
  ...
  def __init__(self, num_z, num_ftr, num_c, arch, mirror):
    ...
    if self.use_mix_log_cdf:
      self.param = MixLogCDFParam(
          num_z, num_mix=3, num_ftr=self.conv.hidden_dim, mirror=mirror)
    else:
...
class HyperAutoEncoder(nn.Module):
  ...
  def __init__(self, args, writer, arch_instance, hyper_cfg):
    ...
    c_scaling = CHANNEL_MULT**(
        self.num_preprocess_blocks + self.num_latent_scales - 1)
    spatial_scaling = 2**(
        self.num_preprocess_blocks + self.num_latent_scales - 1)
  ...
  def init_encoder_tower(self, mult):
    ...
    for s in range(self.num_latent_scales):
      for g in range(self.groups_per_scale[s]):
        ...
        if not (s == self.num_latent_scales - 1 and
                g == self.groups_per_scale[s] - 1):
          num_ce = int(self.num_channels_enc * mult)
          num_cd = int(self.num_channels_dec * mult)
          cell = EncCombinerCell(
              num_ce, num_cd, num_ce, cell_type='combiner_enc', hyper_cfg=self.hyper_cfg)
          enc_tower.append(cell)
      ...
      if s < self.num_latent_scales - 1:
        ...
        num_co = int(CHANNEL_MULT * num_ci)
        cell = HyperCell(
            num_ci, num_co, cell_type='down_enc', arch=arch, use_se=self.use_se, hyper_cfg=self.hyper_cfg)
        enc_tower.append(cell)
        mult = CHANNEL_MULT * mult
  ...
  def init_encoder0(self, mult):
    num_c = int(self.num_channels_enc * mult)
    hyper = get_hyper_layer(num_c, self.hyper_cfg)
  ...
  def init_normal_sampler(self, mult):
    ...
    for s in range(self.num_latent_scales):
      for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
        ...
        if not (s == 0 and
                g == 0):  # for the first group, we use a fixed standard Normal.
    ...
    for s in range(self.num_latent_scales):
      for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
        ...
        cell = DecCombinerCell(
            num_c, self.num_latent_per_group, num_c, cell_type='combiner_dec', hyper_cfg=self.hyper_cfg)
        dec_tower.append(cell)
      ...
      if s < self.num_latent_scales - 1:
        ...
        cell = HyperCell(
            num_ci, num_co, cell_type='up_dec', arch=arch, use_se=self.use_se, hyper_cfg=self.hyper_cfg)
        dec_tower.append(cell)
        mult = mult / CHANNEL_MULT
  ...
  def _sample(self, x: torch.Tensor) -> dict:
    try:
  ...
  def sample_forward(self, x):
    ...
    for cell in self.dec_tower:
      if cell.cell_type == 'combiner_dec':
        if idx_dec > 0:
          ...
          # form encoder
          ftr = combiner_cells_enc[idx_dec - 1](combiner_cells_s[idx_dec - 1],
                                                s)
    ...
    return logits, log_q, log_p, kl_all, kl_diag, sample_dict

  def fixed_forward(self, x, value):
  ...
  def sample(self, num_samples, t, value):
    scale_ind = 0
    z0_size = [num_samples] + self.z0_size

    sample_dict = self.sample_inverse(torch.zeros(z0_size).cuda(), value)
    self.set_net_inputs(sample_dict["net"])
    ...
    if self.dataset in {'mnist', 'omniglot'}:
      ...
      if self.num_mix_output == 1:
        ...
        return DiscMixLogistic(
            logits, self.num_mix_output, num_bits=self.num_bits)
    else:
