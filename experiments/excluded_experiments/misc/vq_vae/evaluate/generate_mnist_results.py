...
ENTITY = "bae-group"
BASELINE_NAME = "hvqvae_image_jobs_final"
HYPER_NAME = "hvqvae_image_hyper_sweep_v14"
ID = "12feup6l"
...
def get_baseline_summary(config_lst,
  ...
  for i, c in enumerate(config_lst):
    if c["data_name"] == "mnist":
      if test:
        if c["seed"] == 0 and 0.1 < c["lamb"] < 10:
          beta_to_rate[c["lamb"]] = summary_lst[i]["test/vq_loss"]
          beta_to_dist[c["lamb"]] = summary_lst[i]["test/recon_loss"]
          beta_to_elbo[c["lamb"]] = summary_lst[i]["test/loss"]
      else:
        if c["seed"] == 0:
          beta_to_rate[c["lamb"]] = summary_lst[i]["train_eval/vq_loss"]
          beta_to_dist[c["lamb"]] = summary_lst[i]["train_eval/recon_loss"]
          beta_to_elbo[c["lamb"]] = summary_lst[i]["train_eval/loss"]
