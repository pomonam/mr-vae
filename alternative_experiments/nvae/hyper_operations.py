...
from temp_utils import average_tensor
...
class HyperConv2D(nn.Conv2d):

    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1,
                 groups=1, bias=False, data_init=False, weight_norm=True, decoder=False):
        super(HyperConv2D, self).__init__(C_in, C_out, kernel_size, stride, padding, dilation, groups, bias)
...
class ELUConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
        ...
        stride = abs(stride)
        self.conv_0 = HyperConv2D(C_in, C_out, kernel_size, stride=stride, padding=padding, bias=True, dilation=dilation,
                                  data_init=True)
...
class BNELUConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.conv_0 = HyperConv2D(C_in, C_out, kernel_size,
                                  stride=stride, padding=padding, bias=True, dilation=dilation)
...
class BNSwishConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.conv_0 = HyperConv2D(C_in, C_out, kernel_size, stride=stride,
...
class EncCombinerCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout, cell_type):
        ...
        self.conv = HyperConv2D(Cin2, Cout, kernel_size=1, stride=1, padding=0, bias=True)

    def forward(self, x1, x2, beta):
        x2 = self.conv(x2, beta)
        out = x1 + x2
        return out
...
class DecCombinerCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout, cell_type):
        ...
        self.cell_type = cell_type
        self.conv = HyperConv2D(Cin1 + Cin2, Cout, kernel_size=1, stride=1,
                                padding=0, bias=True, decoder=True)

    def forward(self, x1, x2, beta):
        out = torch.cat([x1, x2], dim=1)
        out = self.conv(out, beta)
        return out
...
class ConvBNSwish(nn.Module):
    def __init__(self, Cin, Cout, k=3, stride=1, groups=1, dilation=1):
        ...
        self.conv = HyperConv2D(Cin, Cout, k, stride, padding, groups=groups, bias=False,
                                dilation=dilation, weight_norm=False, decoder=True)
        self.bn = SyncBatchNormSwish(Cout, eps=BN_EPS, momentum=0.05)

    def forward(self, x, beta):
        return self.bn(self.conv(x, beta))
...
class InvertedResidual(nn.Module):
    def __init__(self, Cin, Cout, stride, ex, dil, k, g):
        ...
        self.layer1 = get_batchnorm(Cin, eps=BN_EPS, momentum=0.05)
        self.layer2 = ConvBNSwish(Cin, hidden_dim, k=1)
        self.layer3 = ConvBNSwish(hidden_dim, hidden_dim, stride=self.stride, groups=groups, k=k, dilation=dil)
        self.layer4 = HyperConv2D(hidden_dim, Cout, 1, 1, 0, bias=False, weight_norm=False, decoder=True)
        self.layer5 = get_batchnorm(Cout, momentum=0.05)
        self.conv = nn.Sequential(*layers0)

    def forward(self, x, beta):
        x = self.conv(x)
        x = self.layer1(x)
        x = self.layer2(x, beta)
        x = self.layer3(x, beta)
        x = self.layer4(x, beta)
        x = self.layer5(x)
        return x
