...
from experiments.text_legacy.old_version.hyper.layers.scale import HyperScale
from experiments.text_legacy.old_version.hyper.layers.linear import HyperLinear
from experiments.text_legacy.old_version.hyper.models import BaseHyperEncoder
from experiments.text_legacy.old_version.hyper.models import BaseHyperDecoder
...
class HyperLstmEncoder(BaseHyperEncoder):
    ...
    def __init__(self, vocab_size, ni, nh, hyper_config):
        ...
        # self.linear_proj = nn.Linear(nh, nh, "none", hyper_config)
...
class HyperLstmDecoder(BaseHyperDecoder):
    ...
    def special_decode(self, input, z):
        ...
        # self.pred_linear._net_inputs = self.pred_linear._net_inputs.unsqueeze(1).repeat(1, output.shape[1], 1).reshape(-1, 32)
