...
def plot_analytical_post(mean, covariance, beta, hparams):
  ...
  print("analytical q mu at beta={} is {}".format(beta,
                                                  mu.detach().cpu().numpy()))
  print("analytical q cov at beta={} is {}".format(beta,
...
def analytical_rate_distortion(decoder_weights,
  ...
  for i, (data, _) in enumerate(rd_data, 1):
    ...
    if hparams.model_train.z_size == 2:
      plot_analytical_post(q_mean, q_cov, 1. / hparams.messenger.beta, hparams)
...
def run_analytical_rd(model, hparams, data, rd_data_loader, writer):
  ...
  for i in range(hparams.rd.num_betas):
    ...
    with torch.no_grad():
      ...
      if beta == 1:
        log_down_likelihood(analytic_rate, analytic_distortion, data, hparams)
