...
def _sample_inverse(x: torch.Tensor, value: float) -> dict:
    try:
...
def _sample(x: torch.Tensor) -> dict:
    batch_size = x.shape[0]
    device = x.device
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        if epoch % eval_freq == 0 or epoch == (args.epochs - 1):
            ...
            for beta in betas:
                ...
                writer.add_scalar('val/neg_log_p_{}'.format(beta), valid_neg_log_p, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', type=str, default='hyper_checkpoints/',
