...
class CNNDecoder(BaseDecoder):
  ...
  def __init__(self):
    ...
    self.initial_layer = nn.Linear(32, 32 * 8)
...
class PixelCNNDecoder(BaseDecoder):
  ...
  def decode(self, z):
    ...
    img = torch.nn.Variable(z.data.new(batch_size, self.nc, H, W).zero_(), volatile=True)
    # [batch, nc+fm, H, W]
    ...
    for i in range(H):
      for j in range(W):
        ...
        # [batch, nc]
        img[:, :self.nc, i, j] = torch.ge(recon_img[:, :, i, j], 0.5).float() if deterministic else torch.bernoulli(
          recon_img[:, :, i, j])
