...
def main(args):
  ...
  logging.info('groups per scale: %s, total_groups: %d',
               model.groups_per_scale,
               sum(model.groups_per_scale))
...
def train(train_queue,
  ...
  # rate = utils.AvgrageMeter()
  # distortion = utils.AvgrageMeter()
  model.train()
  for step, x in enumerate(train_queue):
    ...
    if (global_step + 1) % 100 == 0:
      ...
      log_dict["train/norm_loss"] = norm_loss
      log_dict["train/bn_loss"] = bn_loss
      log_dict["train/norm_coeff"] = wdn_coeff

      utils.average_tensor(nelbo.avg, args.distributed)
      logging.info('train %d %f', global_step, nelbo.avg)
      # writer.add_scalar('train/nelbo_avg', nelbo.avg, global_step)
      log_dict["train/nelbo_avg"] = nelbo.avg
      ...
      for i, kl_diag_i in enumerate(kl_diag):
        ...
        log_dict['kl/active_%d' % i] = num_active
        log_dict['kl_coeff/layer_%d' % i] = kl_coeffs[i]
        log_dict['kl_vals/layer_%d' % i] = kl_vals[i]

      # writer.add_scalar('kl/total_active', total_active, global_step)
      log_dict['kl/total_active'] = total_active
...
def evaluate(valid_queue, model, num_samples, args, logging):
  ...
  rate_avg = utils.AvgrageMeter()
  dist_avg = utils.AvgrageMeter()
  ...
  for step, x in enumerate(valid_queue):
    ...
    with torch.no_grad():
      ...
      for k in range(num_samples):
        ...
        output = model.decoder_output(logits)
        recon_loss = utils.reconstruction_loss(
            output, x, crop=model.crop_output)
        balanced_kl, _, _ = utils.kl_balancer(kl_all, kl_balance=False)
        nelbo_batch = recon_loss + balanced_kl
        rate_lst.append(balanced_kl)
        dist_lst.append(recon_loss)
      ...
      log_p = torch.mean(
          ...
          np.log(num_samples))
      rate_lst = torch.mean(torch.stack(rate_lst, dim=1))
      dist_lst = torch.mean(torch.stack(dist_lst, dim=1))

    nelbo_avg.update(nelbo.data, x.size(0))
    neg_log_p_avg.update(-log_p.data, x.size(0))
    rate_avg.update(rate_lst.data, x.size(0))
    dist_avg.update(dist_lst.data, x.size(0))

  utils.average_tensor(nelbo_avg.avg, args.distributed)
  utils.average_tensor(neg_log_p_avg.avg, args.distributed)
  utils.average_tensor(rate_avg.avg, args.distributed)
  utils.average_tensor(dist_avg.avg, args.distributed)
  ...
  logging.info('val, step: %d, NELBO: %f, neg Log p %f',
               ...
               nelbo_avg.avg,
               neg_log_p_avg.avg)
  log_dict["eval/nelbo"] = nelbo_avg.avg
  log_dict["eval/neg_log_p"] = neg_log_p_avg.avg
  log_dict["eval/rate"] = rate_avg.avg
  log_dict["eval/distortion"] = dist_avg.avg
  wandb.log(log_dict)
...
def create_generator_vae(model, batch_size, num_total_samples):
  ...
  for i in range(num_iters):
    with torch.no_grad():
      ...
      output = model.decoder_output(logits)
      output_img = output.mean if isinstance(
          output, torch.distributions.bernoulli.Bernoulli) else output.mean()
    yield output_img.float()


def _test_vae_fid(model, args, total_fid_samples):
  dims = 2048
  device = 'cuda'
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--learning_rate', type=float, default=1e-2, help='init learning rate')
  parser.add_argument(
      '--learning_rate_min', type=float, default=1e-4, help='min learning rate')
  parser.add_argument(
  ...
  parser.add_argument(
      '--node_rank', type=int, default=0, help='The index of node.')
  parser.add_argument(
  ...
  if size > 1:
    ...
    for rank in range(size):
      ...
      p = Process(
          target=init_processes, args=(global_rank, global_size, main, args))
      p.start()
      processes.append(p)
