...
from neural_ar_operations import ARInvertedResidual
from neural_ar_operations import ELUConv as ARELUConv
from neural_ar_operations import mix_log_cdf_flow
from neural_ar_operations import MixLogCDFParam
from neural_operations import Conv2D
from neural_operations import DecCombinerCell
from neural_operations import EncCombinerCell
from neural_operations import get_skip_connection
from neural_operations import OPS
from neural_operations import SE
import numpy as np
  ...
  def init_encoder_tower(self, mult):
    ...
    for s in range(self.num_latent_scales):
      for g in range(self.groups_per_scale[s]):
        ...
        if not (s == self.num_latent_scales - 1 and
          ...
          cell = EncCombinerCell(
              num_ce, num_cd, num_ce, cell_type='combiner_enc')
          enc_tower.append(cell)
      ...
      if s < self.num_latent_scales - 1:
        ...
        num_co = int(CHANNEL_MULT * num_ci)
        cell = Cell(
            num_ci, num_co, cell_type='down_enc', arch=arch, use_se=self.use_se)
        enc_tower.append(cell)
        mult = CHANNEL_MULT * mult
  ...
  def init_decoder_tower(self, mult):
    ...
    for s in range(self.num_latent_scales):
      ...
      if s < self.num_latent_scales - 1:
        ...
        cell = Cell(
            num_ci, num_co, cell_type='up_dec', arch=arch, use_se=self.use_se)
        dec_tower.append(cell)
        mult = mult / CHANNEL_MULT
