...
from neural_operations import ConvBNSwish
...
AROPS = OrderedDict([
    ('conv_3x3',
     lambda C,
     masked,
     zero_diag: ELUConv(C, C, 3, 1, 1, masked=masked, zero_diag=zero_diag))
])
...
class Identity(nn.Module):
  ...
  def __init__(self, masked, zero_diag):
    ...
    if zero_diag:
      raise ValueError(
          'Skip connection with zero diag is just a zero operation.')
...
def create_conv_mask(kernel_size, c_in, g_in, c_out, zero_diag, mirror):
  m = (kernel_size - 1) // 2
  mask = np.ones([c_out, c_in // g_in, kernel_size, kernel_size],
                 dtype=np.float32)
  mask[:, :, m:, :] = 0
  mask[:, :, m, :m] = 1
...
class ARConv2d(nn.Conv2d):
  ...
  def __init__(self,
    ...
    """
        ...
        """
    super(ARConv2d, self).__init__(C_in,
                                   C_out,
                                   kernel_size,
                                   stride,
                                   padding,
    ...
    if self.masked:
      ...
      self.mask = torch.from_numpy(
          create_conv_mask(kernel_size, C_in, groups, C_out, zero_diag,
                           mirror)).cuda()
    ...
    init = torch.log(
        norm(self.weight * init_mask, dim=[1, 2, 3]).view(-1, 1, 1, 1) + 1e-2)
