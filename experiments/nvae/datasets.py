...
def get_custom_loaders(args):
  ...
  if args.dataset == 'mnist':
    train_queue = load_mnist_data("train", args.batch_size,
                                  workers=8, data_path="../../logs/data", add_padding=True)
    valid_queue = load_mnist_data("test", args.batch_size,
                                  workers=1, data_path="../../logs/data", add_padding=True)
...
def load_omniglot(data_dir):
  ...
  train_data = 255 * omni['data'].astype('float32').reshape(
      (28, 28, -1)).transpose((2, 1, 0))
  test_data = 255 * omni['testdata'].astype('float32').reshape(
      (28, 28, -1)).transpose((2, 1, 0))
...
def get_loaders_eval(dataset, args):
  ...
  if dataset == 'cifar10':
    ...
    if dataset == 'celeba_64':
      ...
      train_data = LMDBDataset(
          root=args.data, name='celeba', train=True, transform=train_transform)
      valid_data = LMDBDataset(
          root=args.data, name='celeba', train=False, transform=valid_transform)
    else:
    ...
    if dataset.startswith('lsun_bedroom'):
      ...
      train_data = LSUN(
          root=args.data, classes=['bedroom_train'], transform=train_transform)
      valid_data = LSUN(
          root=args.data, classes=['bedroom_val'], transform=valid_transform)
      ...
      train_transform, valid_transform = _data_transforms_lsun(resize)
      train_data = LSUN(
          root=args.data, classes=['tower_train'], transform=train_transform)
      valid_data = LSUN(
          root=args.data, classes=['tower_val'], transform=valid_transform)
    else:
      raise NotImplementedError
    ...
    train_transform, valid_transform = _data_transforms_generic(resize)
    train_data = LMDBDataset(
        root=args.data, name='ffhq', train=True, transform=train_transform)
    valid_data = LMDBDataset(
        root=args.data, name='ffhq', train=False, transform=valid_transform)
  else:
    raise NotImplementedError
