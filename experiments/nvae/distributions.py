...
class DiscLogistic:
  ...
  def __init__(self, param):
    ...
    self.log_scales = torch.clamp(
        param[:, self.num_c:, :, :], min=-8.0)  # B, 3, H, W
  ...
  def sample(self):
    u = torch.Tensor(self.means.size()).uniform_(1e-5,
                                                 1. - 1e-5).cuda()  # B, 3, H, W
...
class DiscMixLogistic:
  ...
  def __init__(self, param, num_mix=10, num_bits=8):
    ...
    self.log_scales = torch.clamp(
        l[:, :, num_mix:2 * num_mix, :, :], min=-7.0)  # B, 3, M, H, W
    self.coeffs = torch.tanh(l[:, :,
                               2 * num_mix:3 * num_mix, :, :])  # B, 3, M, H, W
  ...
  def sample(self, t=1.):
    ...
    sel = one_hot(
        torch.argmax(self.logit_probs / t + gumbel, 1), self.num_mix,
        dim=1)  # B, M, H, W
